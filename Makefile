...
C_TEST_PROGRAM += test
TOSMAKE_PRE_EXE_DEPS += $(PYTHON_MSG_FILES) $(C_MSG_HEADER_FILES) $(C_TEST_PROGRAM)
C_INCLUDE += -I$(TINYOS_ROOT_DIR)/tools/tinyos/c/sf $(CFLAGS)
TOSMAKE_CLEAN_EXTRA += test $(PYTHON_MSG_FILES) $(C_MSG_HEADER_FILES) *.o CB*.c *.pyc
...
$(C_TEST_PROGRAM): $(C_MSG_HEADER_FILES) $(C_TEST_PROGRAM).c
	...
	gcc $(C_INCLUDE) -g -O2 -o $(C_TEST_PROGRAM) $(C_TEST_PROGRAM).o $(TINYOS_ROOT_DIR)/tools/tinyos/c/sf/libmote.a -lpthread CBSweepDataMsg.o CBChannelMaskMsg.o CBRepoQueryMsg.o
